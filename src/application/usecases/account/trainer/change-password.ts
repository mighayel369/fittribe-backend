...
@injectable()
export class ChangeTrainerPasswordUseCase implements IChangePasswordUseCase {
  constructor(
  ...
  async execute(trainerId: string, data: ChangePasswordRequestDTO): Promise<void> {
    ...
    const isCurrentPasswordValid = await this._securityService.comparePassword(oldPassword, trainer.password);
    if (!isCurrentPasswordValid) {
      throw new AppError(ERROR_MESSAGES.CURRENT_PASSWORD_INCORRECT, HttpStatus.UNAUTHORIZED
      );
    }

    const hashedPassword = await this._securityService.hashPassword(newPassword);
    await this._trainerRepository.updatePassword(trainerId, hashedPassword);
  }
}