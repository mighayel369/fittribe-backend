...
import { IResendOtpUseCase } from "application/interfaces/auth/i-resend-otp.usecase";
...
export class ResendOtpUseCase implements IResendOtpUseCase {
  ...
  async execute(otpRequest: ResendOtpRequestDTO): Promise<void> {
    ...
    if (!isOtpSentSuccessfully) {
      throw new AppError(ERROR_MESSAGES.OTP_GENERATE_ERROR, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}