...
import { TrainerBookingDetailsResponseDTO, TrainerBookingDetailsResponseSchema } from "application/dto/booking/trainer/trainer-booking-details.response.dto";
...
@injectable()
export class FetchBookingDetailsForTrainer
  implements
  IFetchBookingDetails<TrainerBookingDetailsResponseDTO> {
  ...
  async execute(
    bookingId: string
  ): Promise<TrainerBookingDetailsResponseDTO> {
    ...
    const mappedResponse = BookingMapper.toTrainerBookingDetailsResponse(
        bookingRecord,
        chatId
      );

    return TrainerBookingDetailsResponseSchema.parse(mappedResponse);
  }
}