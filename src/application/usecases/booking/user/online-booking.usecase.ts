...
import { BookingSummaryDTO, BookingSummarySchema } from "application/dto/booking/shared/booking-summary.response.dto";
...
export class OnlineBookingUseCase
    ...
    async bookSessionWithTrainer(
        input: OnlineBookingRequestBodyDTO,
        userId: string
        ...
        const isAlreadyBooked =
            await this._bookingRepository.checkAvailability(
        ...
        const mappedSummary =
            BookingMapper.toBookingSummary(
                bookingDetails
            );
    ...
    private async _dispatchSuccessNotifications(
        ...
        const userNotif =
            NotificationMapper.toCreateEntity({
                message:
                    `Payment successful! Your trainer will confirm shortly.`,
