...
@injectable()
export class GetTrainerLeaveMetrics
  implements ITrainerLeaveMetrics {
  ...
  async execute(trainerId: string): Promise<TrainerLeaveMetrics[]> {
    ...
    const trainerUsageData = await this._leaveRepository.getTrainerLeaveCounts(
      trainerId
    );

    return LeaveMapper.toTrainerLeaveMetrics(
        trainerUsageData
      );
  }
}