...
import { FetchProgramInventoryResponseDTO, FetchProgramInventoryResponseSchema } from "application/dto/management/programs-management/program-summary.dto";
...
export class FetchProgramInventory implements IFetchProgramInventory {
  ...
  async execute(queryInput: FetchProgramsQueryDTO): Promise<FetchProgramInventoryResponseDTO> {
    ...
    const { data, totalCount } =
      await this._programRepository.findProgramInventory(
        currentPage,
        limit,
        filter || {}
      );

    return FetchProgramInventoryResponseSchema.parse({

      data: data.map(program =>
        ProgramMapper.toProgramSummaryDTO(program)
      ),
      currentPage,
      totalCount,
