...
export class ModifyProgramSpecs implements IModifyProgramSpecs {
  ...
  async execute(bodyData: ModifyProgramBodyDTO, fileData?: ProgramPictureFileDTO): Promise<void> {

    const { programId, name, description } = bodyData;
    ...
    if (fileData) {
      ...
      newPicUrl = await this._cloudinaryService.getProgramImageUrl(
        fileData,
        safeName
      );
    }
    ...
    try {
      ...
      const isUpdated = await this._programRepository.updateProgramSpecs(
        existingProgram
      );
      ...
      const message = domainError instanceof Error
        ? domainError.message
