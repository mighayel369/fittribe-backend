...
import { TrainerProfileAggregate } from "./types/trainer-aggregate.type";
...
export interface ITrainerRepo {
  ...
  findTrainerDetails(trainerId: string): Promise<TrainerProfileAggregate | null>
  ...
  findAllTrainers(page: number, limit: number, filter: ITrainerFilters): Promise<{ data: TrainerProfileAggregate[]; totalCount: number }>;
