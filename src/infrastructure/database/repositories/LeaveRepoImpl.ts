...
export class LeaveRepository
    ...
    async getAllLeaveRequests(
        ...
        const countRes =
            await this.model.aggregate(countPipeline);
        ...
        const docs =
            await this.model.aggregate<LeaveAggregate>(
                pipeline
            );
    ...
    async isTrainerOnLeave(
        ...
        const startOfSelectedDate =
            new Date(baseDate);

        startOfSelectedDate.setUTCHours(
            0,
            0,
            0,
            0
        ...
        const endOfSelectedDate =
            new Date(baseDate);

        endOfSelectedDate.setUTCHours(
            23,
            59,
            59,
            999
    ...
    async getAllLeaveCount(): Promise<{
        approvalStatus: {
            label: string;
            count: number;
        }[];
    ...
    async findLeaveReport(): Promise<LeaveAggregate[]> {
        ...
        return await this.model.aggregate<LeaveAggregate>([
            {
                $match: {
                    start: {
                        $gte: startDate,
